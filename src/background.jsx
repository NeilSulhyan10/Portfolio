...
export const Vortex = ({ theme = "dark", ...props }) => {
  ...
  const baseRadius = props.baseRadius ?? (isDark ? 1 : 2.7);
  const rangeRadius = props.rangeRadius ?? (isDark ? 2 : 4);
  const baseHue = props.baseHue ?? (isDark ? 220 : 120);
  const rangeHue = 100;
  const noiseSteps = 3;
  const xOff = 0.00125;
  const yOff = 0.00125;
  ...
  const backgroundColor =
    props.backgroundColor ?? (isDark ? "#000000" : "rgba(255,255,255,0.9)");
  ...
  const drawParticle = (x, y, x2, y2, life, ttl, radius, hue, ctx) => {
    ...
    else ctx.strokeStyle = `hsla(${hue}, 100%, 35%, ${fadeInOut(life, ttl) * 5})`;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
  ...
  const renderGlow = (canvas, ctx) => {
    ...
    ctx.globalCompositeOperation = isDark ? "lighter" : "multiply";
    ctx.filter = isDark
      ? "blur(4px) brightness(200%)"
      : "blur(6px) brightness(140%)";
    ctx.drawImage(canvas, 0, 0);
    ctx.restore();
  };
